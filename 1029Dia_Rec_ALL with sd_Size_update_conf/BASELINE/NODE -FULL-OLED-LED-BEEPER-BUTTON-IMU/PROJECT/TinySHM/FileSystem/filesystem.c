...
int Load_Config_NODE_INFO(void)
{
    int i;
    char filename[50];
    char buffer[100];
    ...
    // CONFIG file path
    sprintf(filename, "/%s/%s", liftnode_folders[0], config_files[0]); // NODE_INFO.cfg is the second file in config_files
    ...
    if (fr == FR_OK)
    {
        ...
        if (fr == FR_OK && br > 0)
        {
            ...
            // Find "sensing_rate = " and then extract the number behind it
            char *rate_str = strstr(buffer, "sensing_rate = ");
            if (rate_str != NULL)
            {
                sensing_rate = atoi(rate_str + strlen("sensing_rate = ")); // transfer to int
                printf("Loaded sensing_rate: %d\n", sensing_rate);
            }

            char *duration_str = strstr(buffer, "sensing_duration = ");
            if (duration_str != NULL)
            {
                sensing_duration = atoi(duration_str + strlen("sensing_duration = ")); // transfer to int
                printf("Loaded sensing_duration: %d\n", sensing_duration);
            }
        }
        else
        {
...
int Node_FS_Init(void)
{
    ...
    if(FS_Check() != NODE_SUCCESS)
    {
		...
		if(Load_Config_NODE_INFO() != NODE_SUCCESS)
    {
        return NODE_FAIL;
    }
